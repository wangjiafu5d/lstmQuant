...
public class BackPass {
	...
	private List<Matrix> backTrainHiddenLayer(final List<Matrix> w_hidden_list) {
		...
		Matrix in_trans = Matrix.Factory.zeros(xt.getColumnCount(), 2 * xt.getRowCount());
		in_trans = xt.appendVertically(Ret.LINK, ht_prev).transpose();

		// tanh导数等于1-（tanh）^2
		// δCt = ot*(1-(tanhCt)^2)*δht
		Matrix grad_tanh_ct_out = Matrix.Factory.ones(ct_out.getRowCount(), ct_out.getColumnCount())
				.minus(MathUtil.tanh(ct_out).times(MathUtil.tanh(ct_out)));
		Matrix delta_ct_out = ot.times(grad_tanh_ct_out).times(delta_ht);
		...
		Matrix grad_wf = delta_ct_out.times(ct_prev).times(ft).times(ele1).mtimes(in_trans);
		...
		Matrix grad_wi = delta_ct_out.times(ct_cell).times(it).times(ele2).mtimes(in_trans);
		...
		Matrix ele3 = Matrix.Factory.ones(ct_cell.getRowCount(), ct_cell.getColumnCount())
				.minus(ct_cell.times(ct_cell));
		Matrix grad_wc = delta_ct_out.times(it).times(ele3).mtimes(in_trans);
		...
		Matrix grad_wo = delta_ht.times(MathUtil.tanh(ct_out)).times(ot).times(ele4).mtimes(in_trans);
		...
		Matrix xAppendHt_o = w_hidden_list.get(3).transpose()
				.mtimes(delta_ht.times(MathUtil.tanh(ct_out)).times(ot).times(ele4));
		Matrix xAppendHt_c = w_hidden_list.get(2).transpose().mtimes(delta_ct_out.times(it).times(ele3));
		Matrix xAppendHt_i = w_hidden_list.get(1).transpose().mtimes(delta_ct_out.times(ct_cell).times(it).times(ele2));
		Matrix xAppendHt_f = w_hidden_list.get(0).transpose().mtimes(delta_ct_out.times(ct_prev).times(ft).times(ele1));
		Matrix gradXAppendHt = xAppendHt_o.plus(xAppendHt_c).plus(xAppendHt_i).plus(xAppendHt_f);
		
		delta_xt = gradXAppendHt.selectRows(Ret.LINK, 0);
		delta_ht_pre = gradXAppendHt.selectRows(Ret.LINK, 1);
		return new_w_list;
	}
